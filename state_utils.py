...
# ── v5.5 — Post-loss lock ─────────────────────────────────────────────────────
# After ANY losing trade (SL hit), block ALL new entries — regardless of
# direction or zone — for post_loss_lock_hours (default 6h). This is the
# loss-side mirror of the win zone lock: simple, unconditional, time-based.
#
# State stored in runtime_state.json:
#   last_loss_ts : str — SGT timestamp the loss was recorded ("%Y-%m-%d %H:%M:%S")

def set_last_loss_lock(dt: datetime) -> None:
    """Record the timestamp of the most recent losing trade."""
    update_runtime_state(last_loss_ts=dt.strftime("%Y-%m-%d %H:%M:%S"))
    logger.info("Post-loss lock SET — loss @ %s (entries blocked for the lock window)", dt)


def get_last_loss_lock() -> str | None:
    """Return the stored last-loss timestamp string, or None if not set / cleared."""
    state = load_json(RUNTIME_STATE_FILE, {})
    val = state.get("last_loss_ts")
    return val if val else None


def clear_last_loss_lock() -> None:
    """Explicitly clear the post-loss lock."""
    update_runtime_state(last_loss_ts=None)
    logger.info("Post-loss lock CLEARED — entries re-enabled")
